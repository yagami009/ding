...
class GCCA:
    ...
    def fit(self, X):
        ...
        Chi_bar_n = np.mean(
            Chi_n, axis=-1
        )  # mean over trials for each channel with all samples: output shape is Nc x Ns x 1
        Chi_bar_n_c = np.concatenate(
            [Chi_bar_n for i in range(self.Nt)], axis=1
        )  # concat along columns

        Y_n = cca_reference([self.f_ssvep], self.fs, self.Ns, Nh=self.Nh).reshape(
            -1, self.Ns
        )
        ...
        w_Chi_bar_n = w[
            self.Nc : 2 * self.Nc
        ]  # second Nc weights correspond to Nc template channels
        w_Y_n = w[
            2 * self.Nc :
...
class GCCA_SSVEP:
    ...
    def fit(self, X):
        ...
        assert (
            len(X.shape) == 4
        ), "Expected 4th order input data tensor: Nf x Nc x Ns x Nt"
        self.Chi = X
        ...
        for n in range(len(self.stim_freqs)):
            ...
            Y_n = cca_reference(
                [self.stim_freqs[n]], self.fs, self.Ns, Nh=self.Nh
            ).reshape(-1, self.Ns)
            self.Y.append(Y_n)
            Y_n_c = np.concatenate([Y_n for i in range(self.Nt)], axis=1)
            ...
            W.append(
                W_eig[:, i]
            )  # optimal spatial filter vector with dim (2*Nc + 2*Nh)
    ...
    def classify(self, X_test):
        if self.W is None:
            raise ValueError(
                "w must be computed using `compute_w` before performing classification."
            )
...
class MsetCCA:
    ...
    def fit(self, chi):
        ...
        assert (
            len(chi.shape) == 3
        ), "Expected 3rd order input data tensor for freq. fm: Nc x Ns x Nt"
        ...
        w = V[:, np.argmax(lam)].reshape(
            (Nt, Nc)
        )  # sort by largest eig vals in lam vector. TODO: check reshaping

        self.Y = np.array(
            [w[i, :].T.dot(chi[:, :, i]) for i in range(Nt)]
        )  # form optimised reference matrix
...
class MsetCCA_SSVEP:
    def __init__(self, stim_freqs):
        self.stim_freqs = stim_freqs
        self.models = {
            f: MsetCCA() for f in stim_freqs
        }  # init independent TRCA models per stim freq
    ...
    def fit(self, X_ssvep):
        ...
        assert (
            len(self.stim_freqs) == X_ssvep.shape[0]
        ), "Length of supplied stim freqs does not match first dimension of input data"
    ...
    def classify(self, X_test, method="cca"):
        ...
        return {
            f: self.models[f].compute_corr(X_test, method=method)
...
class CCA:
    ...
    def classify(self, X_test, method="eig"):
        ...
        for f in self.stim_freqs:
            ...
            if method == "eig":
                ...
                Xc, Yc = self.cca_models[f].transform(
                    X_test, Y
    ...
    def cca_eig(X, Y, n_components=1):
        ...
        return sorted(np.sqrt(lam), reverse=True)[
            :n_components
        ]  # return largest n sqrt eig vals
...
def fbcca(eeg, list_freqs, fs, num_harms=3, num_fbs=5):
    ...
    (
        ...
        _,
        num_smpls,
    ) = eeg.shape  # 40 taget (means 40 fre-phase combination that we want to predict)
    ...
    for targ_i in range(num_targs):
        ...
        for fb_i in range(
            num_fbs
        ):  # filter bank number, deal with different filter bank
            testdata = filterbank(test_tmp, fs, fb_i)  # data after filtering
            for class_i in range(num_targs):
                refdata = np.squeeze(
                    y_ref[class_i, :, :]
                )  # pick corresponding freq target reference signal
                ...
                r_tmp, _ = pearsonr(
                    np.squeeze(test_C), np.squeeze(ref_C)
                )  # return r and p_value, use np.squeeze to adapt the API
                r[fb_i, class_i] = r_tmp

        rho = np.dot(
            fb_coefs, r
        )  # weighted sum of r from all different filter banks' result
        tau = np.argmax(
            rho
        )  # get maximum from the target as the final predict (get the index)
...
def cca_reference(list_freqs, fs, Ns, Nh=3, standardise_out=False):
    ...
    for freq_i in range(num_freqs):
        ...
        for harm_i in range(1, Nh + 1):
            ...
            tmp.extend(
                [
                    np.sin(2 * np.pi * tidx * harm_i * stim_freq),
...
def fbcca_realtime(data, list_freqs, fs, num_harms=3, num_fbs=5):
    ...
    THRESHOLD = 2.1
    if (
        abs(rho[result]) < THRESHOLD
    ):  # 2.587=np.sum(fb_coefs*0.8) #2.91=np.sum(fb_coefs*0.9) #1.941=np.sum(fb_coefs*0.6)
